...
async def generate_phase_id():
    # ✅ Find only PHASE-* schedules
    last_phase = await db.schedules.find(
        {"schedule_id": {"$regex": "^PHASE-"}}
    ).sort("created_at", -1).to_list(1)

    # ✅ If no valid PHASE id exists
    if not last_phase:
        return "PHASE-0001"

    last_id = last_phase[0]["schedule_id"]

    try:
        # ✅ Extract number safely
        last_number = int(last_id.replace("PHASE-", ""))
        new_number = last_number + 1

        return f"PHASE-{new_number:04d}"

    except:
        # ✅ If corrupted ID found
        return "PHASE-0001"
...
async def create_schedule(
    ...
    # ✅ Phase ID Auto Generate
    phase_id = await generate_phase_id()

    # ✅ Previous Phase Selected
    if schedule.previous_phase_id:

        prev_phase = await db.schedules.find_one(
            {"schedule_id": schedule.previous_phase_id},
            {"_id": 0}
        )

        if not prev_phase:
            raise HTTPException(status_code=404, detail="Previous phase not found")

        prev_end = datetime.fromisoformat(prev_phase["end_date"])
        new_start = prev_end + timedelta(days=1)

        schedule.start_date = new_start.strftime("%Y-%m-%d")

    elif not schedule.start_date:
        raise HTTPException(
            status_code=400,
            detail="Start date required for first phase"
        )
    ...
    schedule_data = {
        "schedule_id": phase_id,  # ✅ CUSTOM ID
